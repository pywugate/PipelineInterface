function SplitChannelsAndPlanes(filename, filepath, skipT)
% DetectRS_OMETIFF(Detect_Read_Save_forOMETIFF)
...
% ---------------------------------------
% IMPORTANT: putting story short, there is a bug for Femtonics MESc
% software that the first frame of the file is not the first plane
% but last plane for volumetric time series imaging.
% so I discard first frame for all planes normally.
...
%% save individual plane(z) as single file


% do the step for different colours
for c = 1:sizeC 
    fields= strcat('colour', num2str(c));
    clear matdata matxyt
    matdata = zeros([sizeY sizeX sizeZ sizeT-skipT], 'uint16');     % for conversion from struct to matrix
    matxyt = zeros([sizeY sizeX sizeT-skipT], 'uint16');            % for final xyt matrix
    ...
    for z = 1: sizeZ
        ...
        output = strcat (colstr, '\', filename(1:idx-1), zstr, colstr, '.tiff');
        outputpath = strcat (filepath, output);

        switch InverColour
                case 0
                    bfsave(uint16((matxyt)), outputpath, 'XYZTC');                  % --- no inversion of colour
%                     saveastiff(uint16(matxyt), outputpath);
                case 1
                    bfsave(uint16(imcomplement(matxyt)), outputpath, 'XYZTC');      % --- do inversion of colour
%                     saveastiff(uint16(imcomplement(matxyt)), outputpath);
        end
    end
end
