function [M_final,shifts_g,template,options,col_shift] = normcorre_batch_even_uint16_vPingYen(Y,options,template)
...
if col_shift 
    ...
    if strcmpi(options.shifts_method,'fft')
        print_msg; fprintf('FFT shifts will be applied. \n');
    else
        options.shifts_method = 'cubic';
        print_msg; fprintf('Cubic shifts will be applied. \n');
    end
end
...
%%      Elapsed time is 147.902124 seconds. for 844 frames
tic;
prevstr = [];
for it = 1:iter
    for t = 1:bin_width:T
        ...
        for i = lY:-1:1
            ...
            % function register_frame here will output "shifts" and "Mf"
        end
        for i = 1:lY
